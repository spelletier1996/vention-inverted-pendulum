...
//!
inline void DrawPendulum(float cart_position, float pendulum_angle,
                         float y_offset = 0) {
  ...
  float x_center = window_pos.x + window_size.x / 2;
  float y_center = window_pos.y + window_size.y / 2 + y_offset;

  float line_thinkness = 2.0;
  float rect_size = 15.0;
  // cart
  draw_list->AddRect(
